...
class RegistroDocentes(QMainWindow):
    ...
    def venta_acep(self):
            ...
            phone = str(tel)
            ident = str(iden)
            if len(nomb) == 0 or len(tipo) == 0 or len(lim) == 0 or len(tel) == 0 or len(iden) == 0 or len(esta) == 0 or \
                    len(asig) == 0 or len(ciudad) == 0:
                self.mostrar_mensaje("Alerta", "¡Hay campos vacios!", "", QMessageBox.Warning, False)

            else:
                if lim.isdigit():
                   register_docent(name, esta, int(lim), type, phone, ident, asig, ciudad)
                   QMessageBox.information(self, "Informacion","¡Datos registrados con exito!")
                else:
                   QMessageBox.information(self, "Informacion","¡Tipo de dato invalido en el campo 'Limite de horas'!")
