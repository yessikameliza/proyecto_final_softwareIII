...
class RegistroAsignaturas(QMainWindow):
    ...
    def registrar_materias(self):
            ...
            if len(cod) == 0 or len(nombre) == 0 or len(numcred) == 0 or len(numhorsem) == 0 or len(codreq) == 0 or len(
                    sem) == 0:
                self.mostrar_mensaje("Alerta", "¡Hay campos vacios!", "", QMessageBox.Warning, False)
            else:
                if numhorsem.isdigit():
                  register_matter(cod, nombre, int(sem), numcred, codreq, int(numhorsem))
                  QMessageBox.information(self, "Informacion", "¡Datos registrados con exito!")
                else:
                    QMessageBox.information(self, "Informacion", "¡Tipo de dato invalido en el campo 'Num horas por semestre'!")
